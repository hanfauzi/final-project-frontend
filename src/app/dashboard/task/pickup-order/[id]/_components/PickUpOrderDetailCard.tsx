...
type PickUpOrderDetailCardProps = {
  pickUpOrder: PickUpOrder;
};

export default function PickUpOrderDetailCard({ pickUpOrder }: PickUpOrderDetailCardProps) {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(pickUpOrder.id).then(() => {
      setCopied(true);
    ...
    <div className="bg-card border-1 rounded-md p-3">
      <div className="flex flex-col items-center gap-4 text-sm">
          ...
          <div className="flex justify-between w-full">
            ...
            <div className="flex-7/12 text-right overflow-hidden">
              <div className="whitespace-nowrap scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 hover:overflow-x-auto">
                {pickUpOrder.assignedByAdmin?.name ?? "-"}
              </div>
            </div>
          </div>
          <div className="flex justify-between w-full">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between">
              ...
              <button
                ...
                onClick={() => {
                  if (pickUpOrder.customerAddress?.latitude && pickUpOrder.customerAddress?.longitude) {
                    handleOpenMap({
                      latitude: pickUpOrder.customerAddress.latitude,
                      longitude: pickUpOrder.customerAddress.longitude,
                    });
                  } else {
            ...
            <div>{pickUpOrder.customerAddress?.address}</div>
          </div>
          ...
          <div className="flex flex-col gap-1 w-full">
            <div className="flex justify-between w-full">
              ...
              <button
                className="hover:cursor-pointer"
                onClick={() => {
                  if (pickUpOrder.outlet?.latitude && pickUpOrder.outlet?.longitude) {
                    handleOpenMap({
                      latitude: pickUpOrder.outlet.latitude,
                      longitude: pickUpOrder.outlet.longitude,
                    });
                  } else {
                    alert("No location available for this address");
                  }
            ...
            <div>{pickUpOrder.outlet?.address}</div>
          </div>
        ...
        <div className="flex justify-between w-full">
          ...
          <div className="flex-7/12 text-right">{pickUpOrder.price}</div>
        </div>

        <div className="bg-muted p-1 w-full rounded-md border-1 border-border">
          <div className="text-center font-semibold">{pickUpOrder.status}</div>
        </div>
      </div>
    </div>
  );
